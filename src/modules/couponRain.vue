  ...
  props: {
    spritesData: {
      ...
      default: {
        ...
        title: {
          json: CDNURL + '/fe/game/couponrain/title.json',
          name: 'title_000',
          start: 0,
          number: 14
        },
        pig: CDNURL + '/fe/game/couponrain/pig.png',
        logo: CDNURL + '/fe/game/couponrain/logo.png',
  ...
  methods: {
    doAnimate() {
      import('pixi.js').then(PIXI => {
        ...
        let root = document.getElementById('anim-game')
        root.appendChild(app.view);
        app.renderer.autoResize = true
        app.renderer.resize(window.innerWidth, window.innerHeight)
        //通用变量
        let that = this
        let [as_width, as_height, game_start] = [app.screen.width, app.screen.height, false];
        //容器
        let container1 = null, container2 = null, container3 = null;
        //精灵
        let [bg, title, pig, logo, button, cover, red, graphics] = []
        //文本
        ...
        let static_arr1 = that.spritesData.bg
        let static_arr2 = typeof (that.spritesData.title) === 'string' ? that.spritesData.title : that.spritesData.title.json
        let static_arr3 = [
          that.spritesData.pig,
          that.spritesData.logo,
          that.spritesData.button,
          that.spritesData.buttonDown,
          that.spritesData.cover,
          that.spritesData.red
        ]
        ...
        function setup(loader, resources) {
          // 背景
          bg = getNewSpriteImage(that.spritesData.bg, {}, app.stage)//红色背景图片
          //新建容器1, 2, 3
          ...
          //容器一
          pig = getNewSpriteImage(that.spritesData.pig, { y: -as_height * 0.04 }, container1)//猪年大吉图像
          logo = getNewSpriteImage(that.spritesData.logo, { x: as_width * 0.83, y: as_height * 0.02, width: as_width * 0.15 }, container1)
          if (typeof (that.spritesData.title) === 'string') {
            title = getNewSpriteImage(that.spritesData.title, { x: as_width * 0.12, y: as_height * 0.01, width: as_width * 0.76 }, container1)
          } else {
            title = getAnimation(that.spritesData.title.name, 0, 14, { x: as_width * 0.12, y: as_height * 0.01, width: as_width * 0.76 }, container1)
            title.animationSpeed = 0.4
          }

          button = getNewSpriteImage(that.spritesData.button, { x: as_width * 0.1, y: as_height * 0.815, width: as_width * 0.8 }, container1) //按钮
          ...
          //容器三，游戏界面
          cover = getNewSpriteImage(that.spritesData.cover, { y: -as_height * 0.1, height: as_height * 1.1 }, container3)//cover图as_width / 750 * 1334
          //红包
          getNewRed()
        ...
        function getNewSpriteImage(url, args, parent) {
          let texture = PIXI.loader.resources[url].texture
          let sprite = new PIXI.Sprite(texture)
          let [width, height] = [texture.width, texture.height]
          sprite.x = args.x || 0
          sprite.y = args.y || 0
          sprite.width = args.width || app.screen.width
          sprite.height = args.height || (sprite.width / texture.width * texture.height)
          for (let key in args) {
            sprite.key = args.key
          }
        ...
        function getNewRed() {
          for (let i = 0; i < totalDudes; i++) {
            ...
            let dude = getNewSpriteImage(that.spritesData.red, { x: x, y: y, width: as_width * 0.15 }, container3)
        ...
        function onchangeImage() {
          button.texture = PIXI.utils.TextureCache[that.spritesData.buttonDown];
        }
    ...
    listenGameEnd() {
      this.$emit('listenGameEnd', this.gameEnd);
    }
  }
};
</script>
