...
export default {
  ...
  data() {
    return {
      baseUrl: cdnUrl + '/fe/marketing/img/damai/guoqing/',
      style: {
        root: {
          height: this.$innerHeight() + 'px'
        },
        photobg: {
          marginTop: '-' + 0.015 * this.$innerHeight() + 'px'
        },
        bg: {
          marginTop: '-' + 0.015 * this.$innerHeight() + 'px'
        },
        photo: {
          top: 0.026 * this.$innerHeight() + 'px'
        },
        button: {
  ...
  methods: {
    ...
    handleNavi() {
      window.location.href = process.env.AD_API+'/api/s/y8W'
    }
  }
}
</script>
...
@baseurl : "http://cdn.xingstation.cn/fe/marketing/img/damai/guoqing/";
.damai {
  width: 100%;
  text-align: center;
  position: relative;
  ...
  .photo-area {
    ...
    .flash {
      animation: scale 0.2s linear infinite alternate;
      width: 23%;
      position: absolute;
      top: -43%;
      left: 0;
